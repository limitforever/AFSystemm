 ...
 *
 */
public class OilTotalStatisticService {

	public Map<String, ArrayList<OilWorkFuelInfo>> GetStatisticOilWorkerInfo() {


		Map<String, ArrayList<OilWorkFuelInfo>> mapLists = new HashMap<String, ArrayList<OilWorkFuelInfo>>();

		Map<String, String[]> monthMap = InitMonthMap();

		java.sql.Connection conn = DBHelper.getConnection();

		String sql = "SELECT workerusername, fuelsheet.workeruserid, SUM(oiltotal) AS OilTotalCount FROM FuelSheet  INNER JOIN OilWorker ON fuelsheet.workeruserid = oilworker.workeruserid WHERE overfueltime >= ? AND overfueltime < ? group by fuelsheet.workeruserid";
		...
		try {
			...
			for (String string : sets) {
				...
				mapLists.put(string,GetStatictisOilWorker(sql, conn, objects));
	...
	public ArrayList<OilWorkFuelInfo> GetStatictisOilWorker(String sql, java.sql.Connection conn,
			Object[] obj) throws SQLException {

		ArrayList<OilWorkFuelInfo> oilWorkFuelInfos = new ArrayList<OilWorkFuelInfo>();
...
//		System.out.println(obj[0].toString()+ obj[1].toString());
		...
		while (resultSet.next()) {
			OilWorkFuelInfo oilWorkFuelInfo = new OilWorkFuelInfo();

			oilWorkFuelInfo.setOilWorkername(resultSet.getString("workerusername"));
			oilWorkFuelInfo.setOilTotal(resultSet.getDouble("OilTotalCount"));
			oilWorkFuelInfos.add(oilWorkFuelInfo);
			
			System.out.println(obj[0]+" 员工姓名: "+oilWorkFuelInfo.getOilWorkername()+" 油量: "+oilWorkFuelInfo.getOilTotal());
		}

		/* Gson gson = new Gson();  		  
			String message2 = gson.toJson(oilWorkFuelInfos);//把对象转为JSON格式的字符串  
			System.out.println("把对象转为JSON格式的字符串(修改)  "+message2); */
		return oilWorkFuelInfos;

	}// function

	public JSONArray GetStatisticOilWorkerData(){
		...
		for (Entry<String, ArrayList<OilWorkFuelInfo>> strings : GetStatisticOilWorkerInfo().entrySet()) {
			
			

			
			for (OilWorkFuelInfo info : strings.getValue()) {
//				 Gson gson = new Gson();  		  
...
//				System.out.println("info" + info.getOilWorkername());
				...
				if (j == 0) {					
					lists.add(info.getOilTotal());
					maplists.put(info.getOilWorkername(), lists);
				}else{
					maplists.get(info.getOilWorkername()).add(info.getOilTotal());
				}
			}
			j  = 1;
