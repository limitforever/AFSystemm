 ...
 *
 */
public class OilTotalAirlineService {

	public Map<String, ArrayList<OilAirlineFuelInfo>> GetStatisticOilAirlineFuelInfo() {


		Map<String, ArrayList<OilAirlineFuelInfo>> mapLists = new HashMap<String, ArrayList<OilAirlineFuelInfo>>();

		Map<String, String[]> monthMap = InitMonthMap();

		java.sql.Connection conn = DBHelper.getConnection();

		String sql = "SELECT alcd, fuelsheet.flid, SUM(oiltotal) AS OilTotalCount FROM FuelSheet  INNER JOIN flightinformation ON fuelsheet.flid = flightinformation.flid WHERE overfueltime >= ? AND overfueltime < ? group by fuelsheet.flid";
		...
		try {
			...
			for (String string : sets) {
				...
				mapLists.put(string,GetStatisticOilAirlineFuel(sql, conn, objects));
	...
	public ArrayList<OilAirlineFuelInfo> GetStatisticOilAirlineFuel(String sql, java.sql.Connection conn,
			Object[] obj) throws SQLException {

		ArrayList<OilAirlineFuelInfo> oilAirlineFuelInfos = new ArrayList<OilAirlineFuelInfo>();
...
//		System.out.println(obj[0].toString()+ obj[1].toString());
		...
		while (resultSet.next()) {
			OilAirlineFuelInfo oilAirlineFuelInfo = new OilAirlineFuelInfo();

			oilAirlineFuelInfo.setAlcd(resultSet.getString("alcd"));
			oilAirlineFuelInfo.setOilTotal(resultSet.getDouble("OilTotalCount"));
			oilAirlineFuelInfos.add(oilAirlineFuelInfo);
	...
	public JSONArray GetStatisticOilWorkerData(){
		...
		for (Entry<String, ArrayList<OilAirlineFuelInfo>> strings : GetStatisticOilAirlineFuelInfo().entrySet()) {
			
			

			
			for (OilAirlineFuelInfo info : strings.getValue()) {
//				 Gson gson = new Gson();  		  
...
//				System.out.println("info" + info.getOilWorkername());
				...
				if (j == 0) {					
					...
					maplists.get(info.getAlcd()).add(info.getOilTotal());
				}
			}
			j  = 1;
