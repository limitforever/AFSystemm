...
<body style="font-family: '幼圆'">
	...
	<div class="easyui-tabs" style="width:1000px;height:550px">
		<div title="加油员信息" style="padding:10px">
		<table id="queryConditionTable">
			
			<tr>
				<td>加油员编号：</td>
				<td><input type="text" id="workeruserid"></td>		    
				<td style="padding-left:20px">加油员姓名：</td>
				
				<td><input type="text" id="workerusername_l"></td>
    ...
    <div id="dlg" class="easyui-dialog" style="width:400px;height:350px;padding:10px 20px"
        ...
        <div class="ftitle">加油员信息</div>
        <form id="fm" method="post" novalidate>
            <div class="fitem">
                <label>加油员编号</label>
                <input name="workeruserid" class="easyui-textbox">
            </div>
            <div class="fitem">
                <label>加油员姓名</label>
                <input name="workerusername" class="easyui-textbox">
            </div>
            <div class="fitem">
                <label>性别</label>                               
    ...
    function getDataGrid()
    {
        ...
        $('#dg').datagrid({    
    	    url:contextPath+'/OilWorkerServlet', 
    	    loadMsg:'正在读取....数据，请稍后...',
    	    pagination : true,
    	    singleSelect : true,
    	    rownumbers:true,
    	    ...
    	    sortName : 'workeruserid',
    	    sortOrder : 'ASC',
    	    columns:[[    
    	        {field:'workeruserid',title:'加油员编号',width:'150px',sortable:true},    
    	        {field:'workerusername',title:'加油员姓名',width:'150px',sortable:true},    
    	        {field:'sex',title:'性别',width:'150px',sortable:true},    
    	        {field:'age',title:'年龄',width:'150px',sortable:true},    
    	        {field:'phonenum',title:'联系方式',width:'150px',sortable:true},    
    	        {field:'repwd',title:'',width:'145px',sortable:true},      
    	    ]]
    ...
    function newUser()
    {            
    	$('#dlg').dialog('open').dialog('setTitle','添加油员');           
    	$('#fm').form('clear');          
    	url = "<%=path %>/OilWorkerAddServlet";        
    }
    ...
    {          
    	...
    	{             
    		...
    		url = contextPath+'/OilWorkerModfiyServlet?id='+row.workerid+'&workeruserid='+row.workeruserid;      
    	}       
    }
            ...
            if (row){
                $.messager.confirm('确认','确认删除该用户?',function(r)             		
                    ...
                    if (r)
                    {
                        ...
                        $.post(contextPath+'/OilWorkerDeleteServlet',{id:row.workerid,workeruserid:row.workeruserid},function(result)
                        {  
